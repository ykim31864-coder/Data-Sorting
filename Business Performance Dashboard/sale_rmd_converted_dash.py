...
def sub_tabs(*tabs: tuple[str, list]) -> dcc.Tabs:
    return dcc.Tabs(
        value=tabs[0][0],
        className="sub-tabs",
        children=[dcc.Tab(label=label, value=label, children=children) for label, children in tabs],
    )
...
sale_aggr["date"] = pd.to_numeric(sale_aggr["date"], errors="coerce")
sale_aggr["date"] = pd.to_datetime(sale_aggr["date"], unit="D", origin="1899-12-30", errors="coerce")
...
def annual_brand_total() -> pd.DataFrame:
    src = brand_hosp_sale_rank.copy()
    if "category" in src.columns:
        src = src[src["category"].astype(str).str.lower().eq("sale")]
    if "HospitalName" in src.columns:
        total_rows = src[src["HospitalName"].astype(str).eq("Total")]
        if not total_rows.empty:
            src = total_rows

    rows = []
    for year in YEAR_COLS:
        total = pd.to_numeric(src[year], errors="coerce").sum()
        without_orapang = pd.to_numeric(src.loc[~src["Brand"].astype(str).str.contains("오라팡", na=False), year], errors="coerce").sum()
        rows.append({"연도": year, "구분": "전체 실적", "실적": total})
        rows.append({"연도": year, "구분": "오라팡 제외 실적", "실적": without_orapang})
    return pd.DataFrame(rows)


def annual_line_fig(title: str = "전체 실적 추이"):
    trend = annual_brand_total()
    fig = px.line(trend, x="연도", y="실적", color="구분", markers=True, title=title)
...
def annual_summary_table() -> pd.DataFrame:
    trend = annual_brand_total()
    pivot = trend.pivot(index="구분", columns="연도", values="실적").reset_index()
    pivot["전년 대비 차이"] = pivot["2026"] - pivot["2025"]
    pivot["전년 대비 성장률"] = (pivot["2026"] / pivot["2025"]) - 1
    pivot["연간 누적 성장률"] = (pivot["2026"] / pivot["2022"]) - 1
    return pivot


def uppdept_annual_line_fig(selected_uppdepts: list[str] | None = None):
    src = hosp_sale_rank.copy()
    src = filter_in(src, "UppDept", selected_uppdepts)
    if "UppDept" in src.columns:
        src = src[src["UppDept"].notna()]
    grouped = src.groupby("UppDept", as_index=False)[YEAR_COLS].sum(numeric_only=True)
    if grouped.empty:
        grouped = pd.DataFrame({"UppDept": [], **{year: [] for year in YEAR_COLS}})
    long = grouped.melt(id_vars="UppDept", value_vars=YEAR_COLS, var_name="연도", value_name="실적")
    fig = px.line(long, x="연도", y="실적", color="UppDept", markers=True, title="본부별 연간 실적 추이")
    fig.update_traces(line={"width": 3}, marker={"size": 7})
    fig.update_layout(xaxis_title="연도", yaxis_title="실적", legend_title_text="본부", hovermode="x unified")
    return fig


def dept_annual_line_fig(selected_depts: list[str] | None = None):
    src = hosp_sale_rank.copy()
    src = filter_in(src, "Dept", selected_depts)
    if "Dept" in src.columns:
        src = src[src["Dept"].notna()]
    grouped = src.groupby("Dept", as_index=False)[YEAR_COLS].sum(numeric_only=True)
    if selected_depts:
        view = grouped
    else:
        view = grouped.assign(total=grouped[YEAR_COLS].sum(axis=1)).nlargest(10, "total").drop(columns="total")
    long = view.melt(id_vars="Dept", value_vars=YEAR_COLS, var_name="연도", value_name="실적")
    fig = px.line(long, x="연도", y="실적", color="Dept", markers=True, title="팀별 연간 실적 추이")
    fig.update_traces(line={"width": 3}, marker={"size": 7})
    fig.update_layout(xaxis_title="연도", yaxis_title="실적", legend_title_text="팀", hovermode="x unified")
    return fig


def brand_annual_line_fig(selected_brands: list[str] | None = None):
    src = brand_hosp_sale_rank.copy()
    if "category" in src.columns:
        src = src[src["category"].astype(str).str.lower().eq("sale")]
    if "HospitalName" in src.columns:
        ...
        if not total_rows.empty:
            src = total_rows
    src = filter_in(src, "Brand", selected_brands)
    grouped = src.groupby("Brand", as_index=False)[YEAR_COLS].sum(numeric_only=True)
    if selected_brands:
        view = grouped
    else:
        view = grouped.assign(total=grouped[YEAR_COLS].sum(axis=1)).nlargest(10, "total").drop(columns="total")
    long = view.melt(id_vars="Brand", value_vars=YEAR_COLS, var_name="연도", value_name="실적")
    fig = px.line(long, x="연도", y="실적", color="Brand", markers=True, title="품목별 연간 실적 추이")
    fig.update_traces(line={"width": 3}, marker={"size": 7})
    fig.update_layout(xaxis_title="연도", yaxis_title="실적", legend_title_text="품목", hovermode="x unified")
    return fig
...
<html>
  <head>
    ...
    <style>
      ...
      .page { max-width: 1480px; margin: 0 auto; padding: 12px 22px 22px; font-family: Arial, sans-serif; }
      .header { background: #2d86df; color: white; display: flex; align-items: center; gap: 18px; padding: 0 16px; min-height: 50px; font-family: Arial, sans-serif; }
      h1 { margin: 0; font-size: 18px; color: white; min-width: 102px; }
      .sub { display: none; }
      .tabs { background: #2d86df; border-bottom: 1px solid #1f6ebc; }
      .tabs .tab { background: #2d86df !important; color: white !important; border: 0 !important; border-right: 1px solid rgba(255,255,255,0.08) !important; padding: 15px 16px !important; font-weight: 800; font-size: 14px; }
      .tabs .tab--selected { background: #0e64ad !important; color: #fff !important; border-top: 0 !important; }
      .sub-tabs { margin-bottom: 12px; border: 1px solid #d4d9df; background: white; }
      .sub-tabs .tab { background: #f8fafc !important; color: #374151 !important; border: 0 !important; border-right: 1px solid #d4d9df !important; padding: 10px 14px !important; font-weight: 800; }
      .sub-tabs .tab--selected { background: #006bb6 !important; color: white !important; }
        ...
        [
            ...
            sub_tabs(
                ...
                (
                    "전체 실적 추이",
                    [
                        html.Div([dcc.Graph(figure=annual_line_fig())], className="section"),
                        table_section("연간 실적 추이 요약", "summary-annual-table", annual_summary_table(), "download-summary-annual"),
                    ],
                ),
                (
                    "품목 요약",
                    [
                        html.Div([dcc.Graph(figure=brand_annual_line_fig())], className="section"),
    ...
    return html.Div(
        [
            sub_tabs(
                ...
                (
                    "본부별 실적 추이",
                    [html.Div([dcc.Graph(id="uppdept-annual-graph", figure=uppdept_annual_line_fig())], className="section")],
                ),
                (
                    "월별 상세",
    ...
    return html.Div(
        [
            sub_tabs(
                ...
                (
                    "팀별 실적 추이",
                    [html.Div([dcc.Graph(id="team-annual-graph", figure=dept_annual_line_fig())], className="section")],
                ),
                (
                    "병원/품목 상세",
    ...
    return html.Div(
        [
            sub_tabs(
                ...
                (
                    "품목별 실적 추이",
                    [html.Div([dcc.Graph(id="brand-annual-graph", figure=brand_annual_line_fig())], className="section")],
                ),
                (
                    "병원별 상세",
...
@app.callback(
    ...
    Output("uppdept-annual-graph", "figure"),
...
def update_uppdept(values):
    ...
    return uppdept_bar_fig(df), uppdept_annual_line_fig(values), table_records(df), table_records(month_df)


@app.callback(
    Output("team-graph", "figure"),
    Output("team-annual-graph", "figure"),
...
def update_team(uppdepts, depts):
    ...
    return dept_bar_fig(graph_df), dept_annual_line_fig(depts), table_records(df), table_records(hosp_item_df)
...
@app.callback(Output("item-graph", "figure"), Output("brand-annual-graph", "figure"), Output("item-table", "data"), Output("item-hospital-table", "data"), Input("item-brand-filter", "value"), Input("item-hospital-filter", "value"), Input("item-year-filter", "value"), prevent_initial_call=True)
def update_item_hospital(brands, hospitals, years):
    ...
    return item_bar_fig(item_df), brand_annual_line_fig(brands), table_records(item_df), table_records(item_hosp_df)
...
DOWNLOADS = {
    ...
    "download-summary-annual": ("summary-annual-table", annual_summary_table(), "연간실적추이요약.csv"),
